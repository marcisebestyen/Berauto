-- InitDb.sql
-- Adatbázis inicializáló szkript
-- Követelmények:
-- 1. Enumok numerikusan (0-tól kezdve C# szerint: Role: Renter=0, Staff=1, Admin=2; FuelType: Diesel=0, Petrol=1, Hybrid=2, Electric=3; RequiredLicence: AM=0, A1=1, A2=2, A=3, B=4).
-- 2. Id-k auto-increment (adatbázis generálja, nem adjuk meg manuálisan).
-- 3. Users.Email oszlop nullázható (FIGYELEM: Ehhez a C# OnModelCreating részben a Users.Email .IsRequired() metódushívást el kell távolítani!).
-- FIGYELEM: Ez a szkript csak akkor működik helyesen, ha az EF Core
-- az enumokat számként (nem stringként) tárolja az adatbázisban,
-- ÉS az Id oszlopok auto-increment tulajdonsággal rendelkeznek,
-- ÉS a Users.Email oszlop engedélyezi a NULL értékeket az adatbázis sémában.
...
VALUES
-- Renter felhasználók (Role = 0)
('Nagy', 'Eleonóra', 'nagy.eleonora', '06301234567', 1, 'AB12345', 0, 'nagy.eleonora@example.com', 'hashed_password_1'), -- Várható ID: 1
('Kis', 'Miklós', 'kis.miklos', '06707654321', 1, 'CD67890', 0, 'kis.miklos@example.com', 'hashed_password_2'),    -- Várható ID: 2
('Kovács', 'Béla', 'kovacs.bela', '06201112233', 0, 'EF54321', 0, 'kovacs.bela@example.com', NULL),                                   -- Várható ID: 3
('Szabó', 'Éva', 'szabo.eva', '06309876543', 1, 'GH09876', 0, 'szabo.eva@example.com', 'hashed_password_3'),        -- Várható ID: 4
('Horváth', 'János', 'horvath.janos', '06701231231', 0, 'IJ21098', 0, 'horvath.janos@example.com', NULL),                                 -- Várható ID: 5
-- Staff/Admin felhasználók
('Admin', 'Felhasználó', 'admin', '06000000000', 1, NULL, 2, 'admin@berauto.com', 'admin_pass_hash'),              -- Várható ID: 6 (Admin)
('Staff', 'Egy', 'staff1', '06000000001', 1, NULL, 1, 'staff1@berauto.com', 'staff1_pass_hash'),                 -- Várható ID: 7 (Staff)
('Staff', 'Kettő', 'staff2', '06000000002', 1, NULL, 1, 'staff2@berauto.com', 'staff2_pass_hash');                -- Várható ID: 8 (Staff)
...
INSERT INTO [dbo].[Cars] ([FuelType], [RequiredLicence], [LicencePlate], [HasValidVignette], [PricePerKilometer], [IsAutomatic], [ActualKilometers], [InProperCondition])
VALUES
(1, 4, 'ABC-123', 1, 150.00, 0, 55000.50, 1), -- Petrol, B (Várható ID: 1)
(0, 4, 'DEF-456', 1, 160.00, 1, 75000.00, 1), -- Diesel, B (Várható ID: 2)
(3, 4, 'GHI-789', 1, 120.00, 1, 22000.75, 1), -- Electric, B (Várható ID: 3)
(1, 4, 'JKL-012', 0, 140.00, 0, 105000.20, 1), -- Petrol, B (Várható ID: 4)
(2, 4, 'MNO-345', 1, 170.00, 1, 15000.00, 0); -- Hybrid, B (Várható ID: 5)
