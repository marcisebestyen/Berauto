    ...
    public class FaqService
    {
        ...
        public async Task<string> GetAnswerFromRAGAsync(string userQuestion)
        {
            ...
            try
            {
                ...
                var ragPrompt =
                    $"A következő kontextus alapján, válaszolj a kérdésre pontosan és barátságosan. Ha a kontextus nem tartalmazza a választ, mondd, hogy nem tudsz válaszolni.\n\nKONTEXTUS:\n{relevantContext}\n\nFELHASZNÁLÓ KÉRDÉSE: {userQuestion}";
            ...
            for (int attempt = 0; attempt < maxRetries; attempt++)
            {
                try
                {
                    ...
                    if (response.StatusCode == System.Net.HttpStatusCode.TooManyRequests)
                    {
                        ...
                        {
                            throw new HttpRequestException($"Rate limit exceeded after {maxRetries} attempts.", null,
                                response.StatusCode);
                        }
                    }
        ...
        private async Task<string> GenerateContentAsync(string prompt)
        {
            ...
            try
            {
                ...
                dynamic generatedGeminiResponse = JsonConvert.DeserializeObject(responseBody) ??
                                                  throw new Exception("Invalid Gemini response.");
            ...
            {
                _logger.LogError(ex,
                    "Error during Gemini content generation (GenerateContentAsync). Server Response: {ResponseBody}",
                    responseBody);
                throw;
            }
        }
